...
data1 = np.loadtxt("NF54_umap.txt")
data2 = np.loadtxt("ap2g_umap.txt")
type1 = np.loadtxt("./real_data/type1.txt")
type2 = np.loadtxt("./real_data/type2.txt")	
...
for i in range(7):
	index1 = np.where(type1==i)	
	plt.scatter(data1[index1,0], data1[index1,1],c=styles[i], s=5.)
fig = plt.figure()	
for i in range(7):
...
for i in range(7):
	index2 = np.where(type2==i)
	plt.scatter(data2[index2,0], data2[index2,1],c=styles[i], s=5.)
plt.show()
